...
class TestLinearHierarchyNodeSetInit(TestBaseLinearHierarchyNodeSet):
    ...
    def test_empty_input(self):
        self.assertRaises(TypeError, nt.NodeChain)

    @clean_up_scene
    def test_none_input(self):
        self.assertRaises(IOError, nt.NodeChain, None)

    @clean_up_scene
    def test_full_list(self):
        nt.NodeChain(self.joints[0])
    ...
    def test_mixed_joints_and_transforms(self):
        nt.NodeChain(self.joints_mixed[0])

    @clean_up_scene
    def test_duplicate(self):
        self.assertEqual(
            len(list(nt.NodeChain(self.joints_total[0], self.joints_total[-3], duplicate=True))),
            len(self.joints_total) - 2)

    @clean_up_scene
    def test_duplicate_no_end_specified(self):
        self.assertEqual(len(nt.NodeChain(self.joints_total[0], duplicate=True,
    ...
    def test_pynode_input(self):
        ...
        self.checkEqual([str(f) for f in list(nt.NodeChain(nodes[0]))],
    ...
    def test_anvil_input(self):
        ...
        self.checkEqual([str(f) for f in list(nt.NodeChain(str(nodes[0])))],
                        [str(f) for f in list(nodes)])
...
class TestLinearHierarchyNodeSetIteration(TestBaseLinearHierarchyNodeSet):
    ...
    def test_specified_end_node(self):
        chain = nt.NodeChain(self.joints_total[0], self.joints_second[-1])
        self.checkEqual(chain, self.joints_second)

    @clean_up_scene
    def test_specified_end_node_with_node_filter(self):
        chain = nt.NodeChain(self.joints_mixed[0], self.joints_total[-3], node_filter=cfg.JOINT_TYPE)
        self.checkEqual(chain, self.joints)

    @clean_up_scene
    def test_specified_end_node_with_node_filter_all(self):
        chain = nt.NodeChain(self.joints_mixed[0], self.joints_total[-3],
                             node_filter=[cfg.JOINT_TYPE, cfg.TRANSFORM_TYPE])
...
class TestHierarchyGetLevel(TestBaseLinearHierarchyNodeSet):
    ...
    def test_joints_only(self):
        chain = nt.NodeChain(self.joints[0])
        self.assertEqual(chain.get_level(4), nt.NodeChain(self.joints[4]).get_hierarchy())

    @clean_up_scene
    def test_joints_mixed(self):
        chain = nt.NodeChain(self.joints_mixed[0])
        self.assertDictEqual(chain.get_level(4), nt.NodeChain(self.joints_mixed[4]).get_hierarchy())

    @clean_up_scene
    def test_with_filter(self):
        filter = [cfg.JOINT_TYPE, cfg.TRANSFORM_TYPE]
        chain = nt.NodeChain(self.joints_total[0], node_filter=filter)
        self.assertDictEqual(chain.get_level(8),
                             nt.NodeChain(self.joints_total[8], node_filter=filter).get_hierarchy())
