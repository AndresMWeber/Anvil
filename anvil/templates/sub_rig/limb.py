...
class Limb(SubRigTemplate):
    ...
    def build(self, parent=None, build_ik=True, meta_data=None, **flags):
        super(Limb, self).build(meta_data=meta_data, parent=parent)
        self.build_internal_structure(build_ik=build_ik)
        self.build_control_structure()
        self.rename()
        self.LOG.info('Built sub rig %s' % self.__class__.__name__)

    def build_internal_structure(self, build_ik=True):
